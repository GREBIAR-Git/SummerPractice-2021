...
namespace Сharts
{
    public partial class MainForm : Form
    {
        ...
        void AreaPaint_Paint(object sender, PaintEventArgs e)
        {
            ...
            Сharts.Paint.DrawLine(e, Color.Black, new Point(0, AreaPaint.Height - 1),
                new Point(AreaPaint.Width, AreaPaint.Height - 1));
            Сharts.Paint.DrawLine(e, Color.Black, new Point(0, 0), new Point(0, AreaPaint.Height));
            Сharts.Paint.DrawLine(e, Color.Black, new Point(AreaPaint.Width - 1, 0),
                new Point(AreaPaint.Width - 1, AreaPaint.Height));
            ...
            if (plusM > 8)
            {
                ...
                {
                    e.Graphics.DrawLine(new Pen(Color.Brown, 1), new Point(i + plusM, 0),
                        new Point(i + plusM, AreaPaint.Height));
                }
                ...
                {
                    e.Graphics.DrawLine(new Pen(Color.Brown, 1), new Point(0, f + plusM),
                        new Point(AreaPaint.Width, f + plusM));
                }
        ...
        void LoadMenuItem_Click(object sender, EventArgs e)
        {
        ...
        void BottomPanelMenuItem_Click(object sender, EventArgs e)
        {
        ...
        void OpenTable(object sender, EventArgs e)
        {
        ...
        void GraphFunctionMenuItem_Click(object sender, EventArgs e)
        {
        ...
        void GraphAdditionalFunctionsMenuItem_Click(object sender, EventArgs e)
        {
            ColorDialog color = new ColorDialog();
            color.AllowFullOpen = false;
            if (color.ShowDialog() == DialogResult.OK)
        ...
        void SlowDrawing_CheckedChanged(object sender, EventArgs e)
        {
        ...
        void SpeedSlow_Scroll(object sender, EventArgs e)
        {
        ...
        void AreaPaint_MouseDown(object sender, MouseEventArgs e)
        {
            if (tableR.Checked)
            {
                ...
                for (int i = 0; i < dopSegments.Count; i++)
                {
                    for (int f = 0; f < dopSegments[i].Length; f++)
                    {
                        if (Math.Round(dopSegments[i][f].X) == coorX && Math.Round(dopSegments[i][f].Y) == coorY)
                        {
                            ...
                            {
                                if (tableA.Rows[z].Cells[0].Value != null && tableA.Rows[z].Cells[1].Value != null &&
                                    float.Parse(tableA.Rows[z].Cells[0].Value.ToString()) == dopSegments[i][f].X &&
                                    float.Parse(tableA.Rows[z].Cells[1].Value.ToString()) == dopSegments[i][f].Y)
                                {
                                    idxPoint = z;
                                    break;
                                }
        ...
        void AreaPaint_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            coordinates.Text = "x = " + coorX + ";  y = " + coorY + ";";
            ...
            if (!slow)
            {
                ...
                graphics.FillEllipse(new SolidBrush(Color.FromArgb(0, 191, 255)),
                    centralX - sizeСircle / 2 + coorX * plusM, centralY - sizeСircle / 2 - coorY * plusM, sizeСircle,
                    sizeСircle);
            }
        }

        void AreaPaint_MouseUp(object sender, MouseEventArgs e)
        {
            if (movePoint)
            {
                float coorX = (float)(e.X - centralX) / plusM;
        ...
        void tableA_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            for (int i = 0; i < dopSegments.Count; i++)
            {
                for (int f = 0; f < dopSegments[i].Length; f++)
                {
                    if (dopSegments[i][f].X == dopPoints[e.RowIndex].X &&
                        dopSegments[i][f].Y == dopPoints[e.RowIndex].Y)
                    {
        ...
        void AreaPaint_MouseLeave(object sender, EventArgs e)
        {
        ...
        void Limitation_TextChanged(object sender, EventArgs e)
        {
            GeneralRestrictions((TextBox)sender);
            AreaPaint.Refresh();
        }

        void PercentScrolling_Scroll(object sender, EventArgs e)
        {
            ForStopSlow();
        ...
        void CentralX_TextChanged(object sender, EventArgs e)
        {
            ForStopSlow();
            CentralXChanged();
        }

        void CentralY_TextChanged(object sender, EventArgs e)
        {
            ForStopSlow();
            CentralYChanged();
        }
        ...
        void PaintChart(Graphics graphics)
        {
            if (!string.IsNullOrEmpty(functionMain.Text))
            {
                ...
                int limitationDownY;
                if (LimitationDownY.Text != "" && int.TryParse(LimitationDownY.Text, out limitationDownY))
                {
                ...
                if (LimitationUpY.Text != "" && int.TryParse(LimitationUpY.Text, out limitationUpY))
                {
                ...
                if (LimitationDownX.Text != "" && int.TryParse(LimitationDownX.Text, out limitationDownX))
                {
                ...
                int limitationUpX;
                if (!int.TryParse(LimitationUpX.Text, out limitationUpX) && LimitationUpX.Text == "")
                {
                    limitationUpX = 10000;
                }

                List<PointF[]> segments = new List<PointF[]>();
                TranslatingExpression translating = new TranslatingExpression();
                for (int i = 0; i < temporaryPoint.Length; i++)
                {
                    temporaryPoint[i] = new PointF(x, translating.Translating(functionMain.Text, x));
                    RemoveUnnecessary(temporaryPoint[i], ref allPoints, ref countAllPoint, limitationDownY,
                        limitationUpY, segments, x);
                    x += 1 / (float)plusM;
                    if (x > limitationUpX)
                    {
                        break;
        ...
        void RemoveUnnecessary(PointF point, ref PointF[] allPoints, ref int countAllPoint, int limitationDownY,
            int limitationUpY, List<PointF[]> segments, float x)
        {
            ...
            if (checkedNumber <= limitationUpY && checkedNumber >= limitationDownY)
            {
                ...
                {
                    TranslatingExpression translating = new TranslatingExpression();
                    PointF pointF = new PointF(point.X - 1 / (float)plusM,
                        translating.Translating(functionMain.Text, point.X - 1 / (float)plusM));
                ...
                foreach (PointF[] points in dopSegments)
                {
                    if (points.Length > 1)
                    {
                        for (int i = 0; i < points.Length - 1; i++)
                        {
                            ...
                            graphics.FillEllipse(new SolidBrush(Color.FromArgb(0, 0, 255)),
                                tempPoints.X - sizeСircle / 2, tempPoints.Y - sizeСircle / 2, sizeСircle, sizeСircle);
                        }
                        ...
                        graphics.FillEllipse(new SolidBrush(Color.FromArgb(0, 0, 255)),
                            tempPointsLast.X - sizeСircle / 2, tempPointsLast.Y - sizeСircle / 2, sizeСircle,
                            sizeСircle);
                    }
                }
            }
                        ...
                        if (points.Length > 1)
                        {
                            ...
                            for (int i = 2; i < points.Length - 2; i++)
                            {
                                if (i == (int)(f + countP))
                                {
                                    ...
                                    tempPoints[countTemp].X =
                                        (float)Math.Round((tempPoints[countTemp].X - centralX) / plusM, 3);
                                    tempPoints[countTemp].Y =
                                        (float)Math.Round(-(tempPoints[countTemp].Y - centralY) / plusM, 3);
                                    ...
                                    if (AdditionalMenuItem.Checked)
                                    {
                                        ...
                                        graphics.FillEllipse(new SolidBrush(Color.FromArgb(0, 0, 255)),
                                            points[i].X - sizeСircle / 2, points[i].Y - sizeСircle / 2, sizeСircle,
                                            sizeСircle);
                                    }
        ...
        void nowTable_Click(object sender, DataGridViewCellMouseEventArgs e)
        {
            AreaPaint.Focus();
        }

        void functionMain_TextChanged(object sender, EventArgs e)
        {
            ForStopSlow();
            if (!string.IsNullOrEmpty(functionMain.Text))
            {
        ...
        void slowMode_Click(object sender, EventArgs e)
        {
            slow = true;
            AreaPaint.Refresh();
        }
        ...
        void CountPoints_TextChanged(object sender, EventArgs e)
        {
            int countpoints;
            if (int.TryParse(CountPoints.Text, out countpoints))
            {
        ...
        void AdditionalMenuItem_Click(object sender, EventArgs e)
        {
        ...
        void tableCompletion()
        {
            ...
            for (int i = 0; x1 <= e1; i++)
            {
                ...
                if (checkedNumber <= limitationUpY && checkedNumber >= limitationDownY)
                {
                    if (countAllPoint < allPoints.Length)
                    {
                        if (!float.IsNaN(temporaryPoint[i].X) && !float.IsNaN(temporaryPoint[i].Y) &&
                            !float.IsInfinity(temporaryPoint[i].Y))
                        {
        ...
        int GeneralRestrictions(TextBox textBox)
        {
            ...
            int inParentheses;
            if (int.TryParse(textBox.Text, out inParentheses))
            {
